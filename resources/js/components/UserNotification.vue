                ...
                <div class="single__items">
                    <div class="miniproduct">
                        ...
                        <div class="item01 d-flex mt--20" v-for="item in unread" :key="item.id">
                            <div class="thumb">
                                <a :href="`edit-comments/${item.data.id}`" @click="readNotifications(item)"><img src="/front-assets/images/icons/comment.png" alt="`${item.data.post_title}`"></a>
                            </div>
                            <div class="content">
                                <a :href="`edit-comments/${item.data.id}`" @click="readNotifications(item)">New comment on your post: {{ item.data.post_title }}</a>
                            </div>
                        </div>
                    </div>
                </div>
